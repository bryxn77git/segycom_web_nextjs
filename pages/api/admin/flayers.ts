...
const createAd = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    try {
        await db.connect();
        const adInDB = await Publicidad.findOne({ name: req.body.name });
        if ( adInDB ) {
            await db.disconnect();
        ...
        const ad = new Publicidad( req.body );
...
const updatedAd = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    try {
        ...
        await db.connect();
        const ad = await Publicidad.findById(_id);
        if ( !ad ) {
            await db.disconnect();
...
const deleteAds = async(req: NextApiRequest, res: NextApiResponse<Data>) => {    
    ...
    try {
        await db.connect();
        const adInDB = await Publicidad.find({ _id: req.body });
        if ( !adInDB ) {
            await db.disconnect();
            return res.status(400).json({ message: 'No hay publicidades con ese o esos ID' });
        }
        
        const adsDelete = await Publicidad.deleteMany({ _id: { $in: req.body }})
        await db.disconnect(); 
